//_____rcc_demo2_________________________________khartinger_____
...
// pin number.
//
// 1. DCC 11, IO expander #1 pin 0: Decoupler (uncoupler)
// 2. DCC 21, IO expander #1 pin 1,2: Two-way switch 
//    (with limit switch)
// 3. DCC 31,32, IO expander #1 pin 3,4,5: Three-way switch
//    (with limit switching)
// 4. DCC 41, IO expander #1 pin 6: Disconnectable track
// 5. DCC 51, IO expander #1 pin 7: Flashing light
// 6. DCC 61, IO expander #3 pin 0,1: Puls 2
//
// The status of the components is shown 
...
// 5. 4x I²C expander boards PCF8574 with the (7-bit) 
//    addresses 0x20 to 0x23
...
// Based on rcc_demo1
// Created by Karl Hartinger, August 11, 2026
// Changes:
// 2026-08-11 New
 ...
 for(int i=0; i<RCOMP_NUM; i++) { // ...for all components.....
  ...
  if(p1.length()>0) {
   ...
   if(aRcomp[i].type==RC_TYPE_P2) {
    switch(aRcmd[i].inValue) { 
     case 0: return T_TRACK_OCC; // 0, BA=00
     case 1: return T_TRACK_FRE; // 1, BA=01
     case 2: return T_TRACK_0V; // 2, BA=10
     case 3: return T_TRACK_00V; // (BA=11)
     default: return "ERR ";    // ?? impossible
    } // END OF switch(aRcmd[i].inValue)
   }
   if(aRcomp[i].type==RC_TYPE_BL) {
    if(aRcmd[i].iCmd==CMD_BLINK) p1+=sON; else p1+=sOFF;
    //aRcmd[i].inValue ? p1+=sON : p1+=sOFF;
...
String setRcompCmd(int iRcomp, int iCmdValue, String sReturn) {
 ...
 if(aRcomp[iRcomp].type==RC_TYPE_UC) {
  ...
  if(iCmdValue==1) {                        // turn current on?
   aRcmd[iRcomp].stateToDo=STATE_NOW;       // now...
