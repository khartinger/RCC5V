...
String simpleGet(String sPayload)
{
 ...
 //-------------------------------------------------------------
  if(sPayload=="version") {
  p1="{\"version\":\""; p1+= String(VERSION_13); p1+="\"}";
  return p1;
 }
 //-------------------------------------------------------------
 if(sPayload=="ip") {
...
void showInfolines() {
 ...
 for(int iPage=0; iPage<iPageMax; iPage++) {
  if(DEBUG_13) { Serial.println("  Show Info Page "+String(1+iPage)); }
...
void setup() {
 ...
 for(int i=0; i<IOEX_NUM; i++) {
  ...
  if(DEBUG_13) { Serial.println(s2); }      // show found addresses
 }
 ...
 if(bUseWiFi) {
  ...
  if(DEBUG_13) Serial.print("setup(): "+s1+String("\n"));
  ...
  do {
   ...
   iUseWiFi--;
   if(DEBUG_13) Serial.println("  Waiting for WiFi: "+String(iUseWiFi));
  } while(!bRet && iUseWiFi>0);
  //.....END OF waiting for WiFi connection.....................
  if(iUseWiFi>0) {
   if(DEBUG_13) Serial.println("setup(): FOUND WiFi " + client.getsSSID());
   //----WiFi ok (no timeout)-----------------------------------
   ...
   {
    iMqttReady--;
    if(DEBUG_13) Serial.println("  Waiting for MQTT: "+String(iMqttReady));
    delay(500);
    client.doLoop();                          // mqtt loop
   };
   if(iMqttReady>0)
   ...
   //----WiFi timeout-------------------------------------------
   if(DEBUG_13) Serial.println("setup(): WiFi " + client.getsSSID() + "NOT FOUND!");
 ...
 if(DEBUG_13) Serial.println("setup(): DccAccessoryDecoder OK");
 //------Finish setup-------------------------------------------
 int iTemp=updateInputValues();
 if(DEBUG_13) Serial.println("setup(): --Finished--\n");
}
